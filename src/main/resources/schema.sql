...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN gr.completed = 1 THEN gr.id END) as completed_games,
    ROUND(AVG(CASE WHEN gr.completed = 1 THEN gr.score_percentage END), 2) as avg_score,
    ROUND(MAX(gr.score_percentage), 2) as best_score,
    MAX(gr.played_at) as last_played
FROM users u
